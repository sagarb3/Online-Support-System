...
<?php
	...
	*/
	...
	/* Get data. */
	$sql = "SELECT * FROM $tbl_name ORDER BY c_id DESC LIMIT $start, $limit";
...
$tfile="answer.php";
	echo '<table height="80%" width="100%" style="border:1px solid black;">'.'<tr style="height:10%">
		<th><h2>Complaint ID</h2></th>
		<th><h2>Complaint Type</h2></th>
		<th><h2>Complaint Status</h2></th>
		<th><h2>User</h2></th>
		<th><h2>Action</h2></th>
	</tr>';
	?>
	...
	echo '<td>'.$row['cmpsts'].'</td>';
	echo '<td>'.$row['user'].'</td>';
	echo '<td>'.'<form action="'.$tfile.'" method="post">'.'<input type="hidden" name="id" value="'.$row['c_id'] .'"/>'.'<input type="submit" value="Proceed" />'.'</form>'.'</td>';
	echo '</tr>';
